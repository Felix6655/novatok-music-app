...
// AI MUSIC GENERATION
const DEMO_PROMPTS = [
  { title: 'Sunset Drive', mood: 'Chill', genre: 'Lo-fi' },
  { title: 'Neon Pulse', mood: 'Energetic', genre: 'Electronic' },
  { title: 'Quiet Storm', mood: 'Dark', genre: 'Cinematic' },
  { title: 'Golden Hour', mood: 'Dreamy', genre: 'Ambient' },
];

// Demo tracks shown when the AI worker is asleep/unavailable - reuses the
// same reliable local audio files as the rest of guest mode.
export const getDemoAITracks = (mood, genre) => {
  const seedTracks = getTracksWithDetails();
  return DEMO_PROMPTS.map((demo, i) => {
    const source = seedTracks[i % seedTracks.length];
    return {
      id: `ai-demo-${i}`,
      title: `${demo.title} (AI Demo)`,
      artist_name: 'NovaTok AI',
...
export const generateAIMusic = async ({ prompt, mood, genre, duration }) => {
  try {
    ...
    if (data.success && data.track) {
      return {
        ...
        track: {
          ...
          cover_url: null,
          duration_sec: data.track.duration_sec || duration || 15,
          mood,
          genre,
          prompt,
          created_at: new Date().toISOString(),
    ...
    return {
      success: false,
      demo: true,
      message: data.message || 'AI generation is in demo mode right now',
      tracks: getDemoAITracks(mood, genre),
    };
  } catch (error) {
...
export const getMyGeneratedTracks = async (userId) => {
  if (isGuestMode()) {
    return getStorage(STORAGE_KEYS.AI_TRACKS, []);
  }
...
export const saveGeneratedTrack = async (track, userId) => {
  if (isGuestMode()) {
    const saved = getStorage(STORAGE_KEYS.AI_TRACKS, []);
    setStorage(STORAGE_KEYS.AI_TRACKS, [track, ...saved.filter(t => t.id !== track.id)]);
    return track;
  }
  ...
  const currentUserId = userId || await getCurrentUserId();
  if (currentUserId === 'guest') throw new Error('Please sign in to save your creation');
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      duration_seconds: track.duration_sec || 0,
    })
    .select()
    .single();

  if (error) throw new Error(`Failed to save track: ${error.message}`);
  return data;
};

// DISCOVER PAGE SECTIONS

export const getCreatorPlaylists = async () => {
  if (isGuestMode()) {
    return artists.map(artist => {
      const artistTracks = getTracksWithDetails().filter(t => t.artist_id === artist.id);
      return {
  ...
  const { data } = await supabase.from('artists').select('*, tracks(*)').limit(8);
...
export const getRecommendedTracks = async (limit = 10) => {
  const allTracks = await getTracks({ limit: 50 });
  // Simple shuffle so "recommended" doesn't just mirror "discover"
  const shuffled = [...allTracks].sort(() => Math.random() - 0.5);
  return shuffled.slice(0, limit);
};

export const getTrendingInReels = async (limit = 8) => {
  const trending = await getTrendingTracks(limit * 2);
  // Sample a different slice than the main trending list so it feels distinct
  return trending.slice(Math.floor(trending.length / 2)).slice(0, limit);
};

const FRIEND_NAMES = ['Maya', 'Jordan', 'Alex', 'Riley', 'Sam', 'Priya', 'Noah', 'Zoe'];

export const getFriendsActivity = async (limit = 6) => {
  const allTracks = await getTracks({ limit: 30 });
  return allTracks.slice(0, limit).map((track, i) => ({
    id: `activity-${track.id}`,
    friend_name: FRIEND_NAMES[i % FRIEND_NAMES.length],
    track,
  }));
};
